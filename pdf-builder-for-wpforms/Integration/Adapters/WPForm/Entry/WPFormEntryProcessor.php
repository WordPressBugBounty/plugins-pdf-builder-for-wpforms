...
use rednaoformpdfbuilder\Integration\Adapters\WPForm\Entry\EntryItems\WPFormRepeaterEntryItem;
...
use rednaoformpdfbuilder\pr\ajax\PDFUtils;
...
class WPFormEntryProcessor extends EntryProcessorBase
{
    public function __construct($loader)
    {
        ...
        add_action('wpforms_pro_admin_entries_edit_submit_completed',array($this,'EditEntry'),1,4);
        add_action('wpforms_entry_details_sidebar_actions',array($this,'GenerateSideBarAction'),10,2);
        add_filter( "bulk_actions-entries", array($this,'AddBulkActions') );
        add_action( 'admin_enqueue_scripts', array($this,'EnqueueScript'));
        add_filter('wpforms_entries_table_process_actions_entries_list',array($this,'HandleBulkAction'),10,2);
    ...
    public function HandleBulkAction( $entries, $args ) {
        if(!$this->Loader->IsPR())
            return $entries;
        $action=sanitize_key( $_GET['action2']);

        switch ($action){
            case 'pdfbuilder_view_pdf':
            case 'pdfbuilder_download_pdf':
                ...
                $formId=$formRepository->GetFormIdFromOriginalId(sanitize_key($_GET['form_id']));
                if($formId==null)
                    return $entries;
                ...
                $pdfUtils=new PDFUtils($this->Loader,$this->Loader->Prefix);
                $pdfUtils->ExecuteBulkAction($action,$entryIds,$templates[0]->Id);
                die();
    ...
    public function AddBulkActions($actions){
        if(!$this->Loader->IsPR())
        {
            $actions['pdfbuilder_view_pdf'] ='Bulk view pdf (Full Version Required)';
            $actions['pdfbuilder_download_pdf'] ='Bulk download (Full Version Required)';
            return $actions;
        }
    ...
    public function GenerateSideBarAction($entry,$formData){
        ...
        if((!current_user_can('administrator')&&!current_user_can('view_entries_pdf'))||!$this->Loader->IsPR())
            return;
    ...
    public function SerializeEntry($entry, $formSettings)
    {
        ...
        foreach($entry->fields as $key=>$value)
        {
            ...
            $fieldEntry=$this->GetEntryItemById($key,$entry,$formSettings);
            $entryItems=array_merge($entryItems,$fieldEntry);
        }
        ...
        if(isset($formSettings))
            ...
            {
                if($currentField->SubType=='content'||$currentField->SubType=='html'||$currentField->Type=='Repeater')
                {
                    $fieldEntry=$this->GetEntryItemById($currentField->Id,$entry,$formSettings);
                    $entryItems=array_merge($entryItems,$fieldEntry);
                }
            }
    ...
    private function GetEntryItemById($fieldId,$entry,$formSettings,$ignoreRepeater=false)
    {
        $entryItems=array();
        $currentField=null;
        foreach($formSettings->Fields as $field)
        {
            $fieldSettingsId=$fieldId;
            if(strpos($fieldSettingsId,'_')!==false)
            {
                $fieldSettingsId=\explode('_',$fieldSettingsId);
                $fieldSettingsId=$fieldSettingsId[0];
            }
            if($field->Id==$fieldSettingsId)
            {
                $currentField=$field;
                break;
            }
        ...
        if(isset($entry->fields[$fieldId]))
        {
            ...
            switch ($currentField->SubType)
            {
                ...
                case 'likert_scale':
                    ...
                    break;
                case 'select':
                    $isDynamic=$currentField->GetAdditionalSetting('Dynamic')!=null;
                    $selectValue = !$isDynamic&&!empty($value['value_raw']) ? $value['value_raw'] : $value['value'];
                    $selectValue = \explode("\n", $selectValue);
            ...
            foreach($formSettings->Fields as $currentField)
            {
                ...
                if($currentField->Type=='Repeater')
                {
                    ...
                    while(true){
                        ...
                        foreach ($currentField->Columns as $column)
                        {
                            foreach ($column->fields as $repeaterField)
                            {
                                if(isset($entry->fields[$repeaterField.$suffix]))
                                {
                                    $currentItem=array_merge($currentItem,$this->GetEntryItemById($repeaterField.$suffix,$entry,$formSettings,true));
                                }
                    ...
                    $entryItems[]=(new WPFormRepeaterEntryItem())->Initialize($currentField)->SetItems($items);
                }
            }
        ...
        if(!$ignoreRepeater)
            for($i=2;true;$i++)
            {
                if(!isset($entry->fields[$fieldId.'_'.$i]))
                    break;

                $items=$this->GetEntryItemById($fieldId.'_'.$i,$entry,$formSettings,true);
                $entryItems=array_merge($entryItems,$items);
        ...
        if(strpos($fieldId,'_')!==false)
        {
            ...
            $index=$fieldId[1];
            foreach($entryItems as $currentEntryItem)
                $currentEntryItem->SetIndex($index);
        }
        return $entryItems;
